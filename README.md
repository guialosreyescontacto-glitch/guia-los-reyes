...
> ⚠️ **Datos de demostración.** Los negocios, teléfonos, correos, sitios web y
...
- **La ficha `basico` es la gratuita**, la que se siembra sin que el negocio la haya pedido: se pinta como un **renglón** y no como tarjeta: sello con el icono de su categoría, nombre, aviso de abierto o cerrado, zona enlazada al mapa, el horario de hoy y el botón de llamar. En el sello va el icono y no las iniciales: dos letras sobre un cuadro de 44px no se leen y no dicen nada —`CM` no es nadie—, mientras que el icono dice de qué es el negocio de un vistazo y es el mismo dibujo con el que la persona acaba de entrar a la categoría. Los renglones salen de la retícula de tarjetas y se juntan en **su propio bloque**, debajo de las que pagan y separado por un filo: metidos en la retícula tenían que ocupar el ancho entero —un renglón no cabe en una columna de tarjeta— y en una pantalla grande quedaban como tiras larguísimas, con el nombre pegado a la izquierda y el botón de llamar perdido en la otra orilla; de paso, la última fila de tarjetas parecía tener un hueco donde faltaba una. Dentro del bloque van **de tres en tres, apiladas una encima de la otra** (`POR_PILA`): cada pila ocupa el lugar de una tarjeta, así que el hueco se llena **por columna y no por renglón**, y en vez de dos fichas sueltas cruzando el ancho se ve otra columna del directorio, que es lo que dice que la lista sigue. El bloque usa **las mismas columnas que las tarjetas** —una, dos desde 700px y tres desde 1080px—, así que cae alineado con la retícula de arriba. Y cuando la última fila de tarjetas queda incompleta —cuatro tarjetas en tres columnas dejan dos lugares—, **el bloque se mete en ese hueco** en vez de abrir renglón nuevo. La cuenta de cuántos lugares sobran se hace **sin una línea de JS**: el bloque es el último hijo de la retícula, así que su número de hijo dice cuántas tarjetas van antes, y un `:nth-child(3n + 2)` distingue el caso de "una tarjeta en la última fila" del de "dos". Al ser CSS, la cuenta se rehace sola cuando cambia el ancho de la ventana, sin escuchar `resize` ni volver a pintar. Sólo se mete al hueco si hay pilas para los lugares que sobraron, ni una más; pasado ese número, o en un navegador sin `:has()`, cae de vuelta al renglón completo de abajo, que siempre se ve bien. En un teléfono caben diez renglones donde antes cabía una tarjeta —una tarjeta con la mitad del contenido seguía ocupando la pantalla de lado a lado y se leía como si valiera lo mismo que la de al lado—. **Sin foto, sin descripción, sin etiquetas, sin calificación y sin WhatsApp, correo ni redes**; sus etiquetas tampoco cuentan en el buscador. Todo eso es justo lo que compra la ficha completa. Los datos siguen escritos en el catálogo aunque no se pinten, así que subir a un negocio de plan es cambiar una palabra.
- **La pila mide exactamente lo que una tarjeta, abierta o cerrada.** Se estira a lo alto de la fila y **el sobrante se lo quedan las fichas, no los huecos**: cada una crece un poco (`flex: 1 1 auto`) y su contenido queda centrado, así que el aire va por dentro —donde se lee como holgura de la ficha— y no por fuera, donde se leía como un espacio olvidado. Entre una ficha y otra quedan 10px fijos. Al desplegar un horario esa ficha pide más y las otras devuelven su parte, así que el conjunto sigue midiendo igual y **no se mueve nada**: ni la tarjeta vecina ni el resto de la página. Medido en Salud a lo largo de varias cargas: 395px, lo mismo que la tarjeta, cerrada y con casi cualquiera de los tres horarios abierto; el peor caso visto crece 14px, cuando al mismo renglón le tocan un nombre largo y una jornada partida —antes de apretar el renglón eran 94px—. Sólo la pila completa se estira; si el último grupo trae una o dos fichas se quedan de su tamaño, juntas arriba, porque estirar dos las dejaría como cajones enormes medio vacíos.
- **En la ficha gratuita el teléfono va a la vista**, en su propio renglón, en los aparatos que no llaman —la misma pregunta que decide el botón redondo, no un punto de quiebre de ancho—. Va **escrito igual que la zona**, icono y texto al hilo del renglón, no en pastilla: en una computadora el número no está para apretarlo, está para leerlo y copiarlo, así que no necesita cuerpo de botón; lo que sí necesita es que se note, y por eso lleva el color de acento y negritas mientras los demás datos van en tinta apagada. Al hacerle clic se copia, con el mismo manejador del botón redondo. El renglón completo lo ocupa un envoltorio, no el enlace: si se lo quedara el enlace, el subrayado punteado de pasar el cursor le cruzaría la ficha de orilla a orilla en vez de quedarse debajo de las cifras, como sí hace el de la zona. En un teléfono no aplica: ahí sigue el botón redondo, porque ahí `tel:` sí marca y con un toque ya está sonando. **Mientras la semana de esa ficha está desplegada el número se quita**: los dos se turnan el renglón, así la tabla entra donde estaba el número en vez de empujar, y al cerrar el horario el número vuelve.
- **Sólo un horario gratuito abierto a la vez**: al abrir uno se cierra el anterior —con el evento `toggle` escuchado en captura, porque no burbujea—. No es sólo comodidad: es parte de la cuenta del alto. Y dentro de la pila el renglón se aprieta, que es la otra parte: el nombre y el aviso se quedan en una línea (el nombre largo se corta con puntos suspensivos y completo sigue en su `title`) y la tabla del horario baja de tamaño. Sin apretarlo, tres fichas con un horario abierto llegaban a 441px contra los 395px de la tarjeta: se salían y estiraban a la vecina.
...
- **Tarjetas de negocio** con banner, etiqueta de plan, aviso de abierto o cerrado, horario y etiquetas. **Sin calificación**: una estrella sólo vale lo que valga quien la puso, y para que signifique algo hacen falta cuentas, moderación y alguien que atienda al negocio que se queja de una reseña injusta —eso es otro producto—. Puesta a mano no era una calificación sino la opinión del que llenó el catálogo, con el nombre de un negocio real debajo.
...
- **Medición de contactos**, que es lo que le da al negocio una razón para renovar: al que paga hay que poder decirle qué recibió, y "el sitio tuvo mil visitas" no es una respuesta. Lo que se cuenta es el **contacto** —cuántas veces le abrieron el WhatsApp, le destaparon el teléfono, le pidieron cómo llegar, le abrieron una red—, negocio por negocio, más los clics en los banners de la portada (que es justo lo que compran el Destacado y el Premium), las categorías y filtros que se abren, los términos que se teclean en el buscador y el plan que se toca en la página de registro. **De quien mira no se guarda nada**: ni cookie, ni identificador, ni rastro de una visita a la siguiente. Por lo mismo **no se cuentan las apariciones** de una ficha en pantalla, sólo los contactos: la aparición no se puede contar sin seguirle la pista a la persona, y además serían cien apuntes por visita en vez de uno. El buscador se apunta cuando la persona **deja de teclear** —"d", "de", "den" y "dent" son la misma búsqueda a medio escribir—, y ahí está el dato más útil para vender: **lo que la gente busca y el directorio no tiene** es la lista de a quién ir a tocarle la puerta. El cableado vive en un solo lugar: cada botón dice por dónde se contactó en un `data-medio` y la ficha que lo contiene dice de quién es en su `data-neg`, así que agregar un botón nuevo no obliga a tocar el medidor. Destapar el número cuenta un contacto y copiarlo después **no cuenta otro**, porque es la misma persona; y el clic que va detrás de un arrastre en el carrusel no cuenta, porque no fue un clic. **A dónde se mandan los apuntes se decide con una sola línea** (`DESTINO` en `app.js`): vacía —como está publicado hoy— la medición queda armada y callada, sin una sola petición a ningún lado. Con `?medicion` en la dirección los apuntes salen por la consola sin mandarse, que es como se revisa que un botón nuevo quedó contado. Ningún evento pasa de **dos datos**, que es el tope del recolector de Vercel.
- **Aviso de privacidad** en `#/privacidad`, enlazado desde el pie. Está escrito para las **dos personas distintas** que toca el directorio, porque no se parecen en nada: el **negocio**, de quien sí se guardan datos y además se publican —esa es la finalidad, y así se dice, sin rodeos—, y **quien nada más entra a ver**, de quien hoy no se recoge nada: sin cuentas, sin formularios y sin cookies. Lo único que se guarda de un visitante vive en su propio aparato (si eligió modo claro u oscuro) y no viaja a ningún lado. Lleva lo que la ley pide: quién es el responsable y dónde encontrarlo, qué datos se guardan, para qué, cuáles finalidades se pueden rechazar sin perder la ficha, a quién se le dan, los cuatro derechos ARCO con su plazo, cómo revocar el consentimiento y cómo se avisan los cambios. También promete que **si se encienden las mediciones, el aviso se actualiza antes y no después**. Los huecos que faltan por llenar —nombre del responsable, domicilio, WhatsApp y fecha— van marcados en **amarillo de marcatextos**, no en gris: un hueco discreto se publica sin que nadie lo note, y un aviso de privacidad con el nombre del responsable en blanco no es un aviso de privacidad. Que estorbe a la vista es la función; se borran solos al llenarlos.
- **Los mensajes para pedir permiso** viven en [`docs/consentimiento.md`](docs/consentimiento.md): la invitación, el mensaje de consentimiento que hay que hacer contestar por escrito antes de dar de alta a nadie, el de "ya quedó, revísalo", el de baja, y qué guardar de cada quien. El aviso de privacidad vive en el sitio, pero el consentimiento se pide en el chat, que es donde de verdad ocurre.
...
```
.
├── index.html              Estructura HTML5 semántica y las 5 vistas
├── assets/
│   ├── favicon.svg
...
- **Llenar los huecos amarillos del aviso de privacidad** (`#/privacidad` en `index.html`): nombre del responsable, domicilio, WhatsApp y fecha. Mientras haya uno sin llenar, el aviso no sirve como aviso.
...
- **Que Google encuentre cada negocio**: hoy todo el directorio vive detrás de un `#`, y lo que va después del `#` no llega al servidor, así que Google ve una sola página. Hacen falta direcciones propias por categoría y por negocio, un `id` fijo en cada uno para que cambiarle el nombre no rompa el enlace, ficha `LocalBusiness` en JSON-LD, `sitemap.xml`, `robots.txt` y `canonical`. Conviene hacerlo **después** de tener negocios reales: abrirle la puerta a Google con datos de ejemplo indexaría 66 negocios que no existen.
