...
- **La ficha `basico` es la gratuita**, la que se siembra sin que el negocio la haya pedido: se pinta como un **renglón** y no como tarjeta: sello con el icono de su categoría, nombre, aviso de abierto o cerrado, zona enlazada al mapa, el horario de hoy y el botón de llamar. En el sello va el icono y no las iniciales: dos letras sobre un cuadro de 44px no se leen y no dicen nada —`CM` no es nadie—, mientras que el icono dice de qué es el negocio de un vistazo y es el mismo dibujo con el que la persona acaba de entrar a la categoría. Los renglones salen de la retícula de tarjetas y se juntan en **su propio bloque**, debajo de las que pagan y separado por un filo: metidos en la retícula tenían que ocupar el ancho entero —un renglón no cabe en una columna de tarjeta— y en una pantalla grande quedaban como tiras larguísimas, con el nombre pegado a la izquierda y el botón de llamar perdido en la otra orilla; de paso, la última fila de tarjetas parecía tener un hueco donde faltaba una. El bloque usa **las mismas columnas que las tarjetas** —una, dos desde 700px y tres desde 1080px—, así que cae alineado con la retícula de arriba y no se ven dos anchos distintos en la misma página. Y cuando la última fila de tarjetas queda incompleta —cuatro tarjetas en tres columnas dejan dos lugares—, **el bloque se mete en ese hueco** en vez de abrir renglón nuevo, así que el espacio no se desperdicia. La cuenta de cuántos lugares sobran se hace **sin una línea de JS**: el bloque es el último hijo de la retícula, así que su número de hijo dice cuántas tarjetas van antes, y un `:nth-child(3n + 2)` distingue el caso de "una tarjeta en la última fila" del de "dos". Al ser CSS, la cuenta se rehace sola cuando cambia el ancho de la ventana, sin escuchar `resize` ni volver a pintar. Sólo se mete al hueco si cabe sin pasarse de alto —hasta 4 renglones en un lugar suelto, hasta 8 en dos—, porque un bloque más alto que la tarjeta estiraría a la tarjeta de al lado y dejaría un hueco peor del que veníamos a quitar; pasado ese número, o en un navegador sin `:has()`, cae de vuelta al renglón completo de abajo, que siempre se ve bien. En un teléfono caben diez donde antes cabía una tarjeta —una tarjeta con la mitad del contenido seguía ocupando la pantalla de lado a lado y se leía como si valiera lo mismo que la de al lado. Sin foto, sin descripción, sin etiquetas, sin calificación y sin WhatsApp, correo ni redes; sus etiquetas tampoco cuentan en el buscador. Todo eso es justo lo que compra la ficha completa. Los datos siguen escritos en el catálogo aunque no se pinten, así que subir a un negocio de plan es cambiar una palabra.
