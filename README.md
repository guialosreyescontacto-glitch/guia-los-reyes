...
- **La ficha `basico` es la gratuita**, la que se siembra sin que el negocio la haya pedido: se pinta como un **renglón** y no como tarjeta: sello con el icono de su categoría, nombre, aviso de abierto o cerrado, zona enlazada al mapa, el horario de hoy y el botón de llamar. En el sello va el icono y no las iniciales: dos letras sobre un cuadro de 44px no se leen y no dicen nada —`CM` no es nadie—, mientras que el icono dice de qué es el negocio de un vistazo y es el mismo dibujo con el que la persona acaba de entrar a la categoría. Los renglones salen de la retícula de tarjetas y se juntan en **su propio bloque**, debajo de las que pagan y separado por un filo: metidos en la retícula tenían que ocupar el ancho entero —un renglón no cabe en una columna de tarjeta— y en una pantalla grande quedaban como tiras larguísimas, con el nombre pegado a la izquierda y el botón de llamar perdido en la otra orilla; de paso, la última fila de tarjetas parecía tener un hueco donde faltaba una. Dentro del bloque van **de tres en tres, apiladas una encima de la otra** (`POR_PILA`): cada pila ocupa el lugar de una tarjeta, así que el hueco se llena **por columna y no por renglón**, y en vez de dos fichas sueltas cruzando el ancho se ve otra columna del directorio, que es lo que dice que la lista sigue. El bloque usa **las mismas columnas que las tarjetas** —una, dos desde 700px y tres desde 1080px—, así que cae alineado con la retícula de arriba. Y cuando la última fila de tarjetas queda incompleta —cuatro tarjetas en tres columnas dejan dos lugares—, **el bloque se mete en ese hueco** en vez de abrir renglón nuevo. La cuenta de cuántos lugares sobran se hace **sin una línea de JS**: el bloque es el último hijo de la retícula, así que su número de hijo dice cuántas tarjetas van antes, y un `:nth-child(3n + 2)` distingue el caso de "una tarjeta en la última fila" del de "dos". Al ser CSS, la cuenta se rehace sola cuando cambia el ancho de la ventana, sin escuchar `resize` ni volver a pintar. Sólo se mete al hueco si hay pilas para los lugares que sobraron, ni una más; pasado ese número, o en un navegador sin `:has()`, cae de vuelta al renglón completo de abajo, que siempre se ve bien.
- **La pila mide exactamente lo que una tarjeta, abierta o cerrada.** Se estira a lo alto de la fila y **el sobrante se reparte entre una ficha y otra** (`justify-content: space-between`), no se queda todo abajo: las tres van de arriba a abajo ocupando lo mismo que la tarjeta de al lado. Al desplegar un horario esos espacios **se encogen solos** —el sobrante es lo que se recorta— así que el conjunto sigue midiendo igual y **no se mueve nada**: ni la tarjeta vecina ni el resto de la página. Sólo la pila completa se reparte así; si el último grupo trae una o dos fichas se quedan juntas arriba, porque repartir dos las dejaría flotando en las orillas.
- **En la ficha gratuita el teléfono va a la vista, debajo del horario**, en los aparatos que no llaman —la misma pregunta que decide el botón redondo—. En la computadora no hay razón para esconderlo detrás de un botón: ese botón existía para no gastar el poco ancho de un teléfono, y aquí el renglón va apilado en una columna y le sobra alto. Puesto a la vista se lee de corrido, se puede teclear en el aparato de a de veras, se copia de un toque —lo hace el mismo manejador del botón— y de paso llena el espacio que quedaba en blanco entre una ficha y otra: con el número, los huecos de la pila bajan de 61px a 13px. En un teléfono no aplica: ahí sigue el botón redondo, porque ahí `tel:` sí marca. **Mientras el horario de esa ficha está desplegado el número se quita**: los dos se turnan el mismo lugar, así la semana entra donde estaba el número en vez de empujar, y al cerrar el horario el número vuelve.
- **Lo que no cabe, honestamente.** Con el número a la vista la pila mide 395px cerrada, exactamente lo que la tarjeta. Al desplegar un horario crece entre 0 y 39px según la ficha —lo peor es una jornada partida, cuya tabla ocupa tres renglones y parte los valores en dos líneas—, y esos pixeles se los come la tarjeta de al lado como blanco al fondo. Tres fichas gratuitas con teléfono ya llenan el alto de una tarjeta, así que no queda margen para la semana desplegada: o el número, o que la semana quepa siempre. Si algún día estorba, la salida limpia es quitarle el desplegable de la semana a la ficha gratuita —dejándole el "Hoy …" y el teléfono— y que el horario completo sea de la ficha de pago; entonces la cuenta cierra siempre. Para que la cuenta salga, **sólo un horario gratuito puede estar abierto a la vez** —al abrir uno se cierra el anterior, con el evento `toggle` escuchado en captura porque no burbujea— y dentro de la pila el renglón se aprieta: el nombre y el aviso se quedan en una línea (el nombre largo se corta con puntos suspensivos y completo sigue en su `title`) y la tabla del horario baja de tamaño. Sin apretarlo, el contenido de tres fichas con un horario abierto llegaba a 441px contra los 395px de la tarjeta: se salía y estiraba a la vecina. Apretado son 370px en el peor caso, que es lo que deja margen para repartir. En el teléfono nada de esto aplica: ahí el renglón tiene el ancho entero y no le falta lugar. En un teléfono caben diez donde antes cabía una tarjeta —una tarjeta con la mitad del contenido seguía ocupando la pantalla de lado a lado y se leía como si valiera lo mismo que la de al lado. Sin foto, sin descripción, sin etiquetas, sin calificación y sin WhatsApp, correo ni redes; sus etiquetas tampoco cuentan en el buscador. Todo eso es justo lo que compra la ficha completa. Los datos siguen escritos en el catálogo aunque no se pinten, así que subir a un negocio de plan es cambiar una palabra.
