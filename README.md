...
- **La pila tiene el espacio apartado de antemano.** Se estira a lo alto de la fila —o sea, a lo que mide la tarjeta de al lado— con las fichas pegadas arriba, así que al desplegar un horario crece dentro de lo que ya tenía y **no empuja nada**: ni la tarjeta vecina ni el resto de la página se mueven. Para que la cuenta salga, **sólo un horario gratuito puede estar abierto a la vez** —al abrir uno se cierra el anterior, con el evento `toggle` escuchado en captura porque no burbujea— y dentro de la pila el renglón se aprieta: el nombre y el aviso se quedan en una línea (el nombre largo se corta con puntos suspensivos y completo sigue en su `title`) y la tabla del horario baja de tamaño. Medido: tres fichas apiladas ocupan 294px cerradas y 370px con un horario abierto, contra los 395px de una tarjeta. En el teléfono nada de esto aplica: ahí el renglón tiene el ancho entero y no le falta lugar. En un teléfono caben diez donde antes cabía una tarjeta —una tarjeta con la mitad del contenido seguía ocupando la pantalla de lado a lado y se leía como si valiera lo mismo que la de al lado. Sin foto, sin descripción, sin etiquetas, sin calificación y sin WhatsApp, correo ni redes; sus etiquetas tampoco cuentan en el buscador. Todo eso es justo lo que compra la ficha completa. Los datos siguen escritos en el catálogo aunque no se pinten, así que subir a un negocio de plan es cambiar una palabra.
