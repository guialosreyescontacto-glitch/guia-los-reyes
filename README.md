...
- **Jerarquía de planes**: los `premium` van hasta arriba con badge de corona, después los `destacado` con badge ámbar, luego las fichas `completa` y al final las `basico`. Dentro de cada grupo se ordenan por calificación. El sello de plan sólo lo llevan los dos que compran posición; la ficha completa se distingue sola, por todo lo que la básica no trae.
- **La ficha `basico` es la gratuita**, la que se siembra sin que el negocio la haya pedido: nombre, zona enlazada al mapa, horario con el aviso de abierto o cerrado, y el botón de llamar. Sin foto, sin descripción, sin etiquetas, sin calificación y sin WhatsApp, correo ni redes; sus etiquetas tampoco cuentan en el buscador. Todo eso es justo lo que compra la ficha completa. Los datos siguen escritos en el catálogo aunque no se pinten, así que subir a un negocio de plan es cambiar una palabra.
...
- **Página de registro** con la escalera de cobro —Ficha básica (gratis), Ficha completa ($150 MXN/mes o $1,500 al año), Destacado ($300 / $3,000) y Premium ($500 / $5,000, uno por giro)— más una caja aparte con los servicios que se cobran una sola vez (alta en Google Maps, fotos, menú con QR, paquete de arranque y sitio web propio con dominio). El precio anual equivale a dos meses gratis. Cada tarjeta lleva su propio botón que abre WhatsApp con el mensaje del plan elegido ya escrito; el de servicios usa `data-mensaje` para mandar otro recado.
